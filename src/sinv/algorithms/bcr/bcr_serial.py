...
def bcr_serial(A: np.ndarray, blocksize: int) -> np.ndarray:
...
def reduce_bcr(
    A: np.ndarray, L: np.ndarray, U: np.ndarray, i_bcr: np.ndarray, blocksize: int
) -> int:
...
def invert_block(
    A: np.ndarray, G: np.ndarray, target_block: int, blocksize: int
) -> None:
...
def produce_bcr(
    ...
    for level_blockindex in range(height - 1, -1, -1):
        ...
        i_from = [
            i
            for i in range(
                int(math.pow(2, level_blockindex + 1)) - 1,
                nblocks,
                int(math.pow(2, level_blockindex + 1)),
            )
        ]
