...
def reduce_schur(A, top_blockrow, bottom_blockrow, blocksize):
    ...
    if comm_rank == comm_size-1:
        ...
        for i_blockrow in range(bottom_blockrow-2, top_blockrow, -1):
            i_rowindice   = i_blockrow*blocksize
            ip1_rowindice = (i_blockrow+1)*blocksize
    ...
    if comm_rank != 0 and comm_rank != comm_size-1 and comm_size > 1:
...
def inverse_hybrid(A, blocksize):
    ...
    if comm_rank == 0:
        ...
        A, L, U = reduce_schur(A, top_blockrow, bottom_blockrow, blocksize)
