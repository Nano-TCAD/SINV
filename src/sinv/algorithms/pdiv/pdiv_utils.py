...
def partition_subdomain(
    ...
    for i in range(len(l_start_blockrow)):
        ...
        if i < len(l_start_blockrow) - 1:
            Bu_i.append(
                A[
                    stop_index - blocksize : stop_index,
                    stop_index : stop_index + blocksize,
                ]
            )
            Bl_i.append(
                A[
...
def send_partitions(K_i: np.ndarray, K_local: np.ndarray):
...
def recv_partitions(K_local: np.ndarray, l_partitions_blocksizes: list, blocksize: int):
...
def invert_partition(
    K_local: np.ndarray, l_partitions_blocksizes: list, blocksize: int
):
    ...
    K_local[start_index:stop_index, start_index:stop_index] = np.linalg.inv(
        K_local[start_index:stop_index, start_index:stop_index]
    )


def compute_full_update_term(
    K: np.ndarray,
    Bu: np.ndarray,
    Bl: np.ndarray,
    phi_1_size: int,
    ...
    U[0:phi_1_size, 0:phi_1_size] = (
        -1
        * K[0:phi_1_size, phi_1_size - blocksize : phi_1_size]
        @ Bu
        @ J12
        @ K[phi_1_size - blocksize : phi_1_size, 0:phi_1_size]
    )
    U[0:phi_1_size, phi_1_size:assembled_system_size] = (
        -1
        * K[0:phi_1_size, phi_1_size - blocksize : phi_1_size]
        @ Bu
        @ J11
        @ K[phi_1_size : phi_1_size + blocksize, phi_1_size:assembled_system_size]
    )
    U[phi_1_size:assembled_system_size, 0:phi_1_size] = (
        -1
        * K[phi_1_size:assembled_system_size, phi_1_size : phi_1_size + blocksize]
        @ Bl
        @ J22
        @ K[phi_1_size - blocksize : phi_1_size, 0:phi_1_size]
    )
    U[phi_1_size:assembled_system_size, phi_1_size:assembled_system_size] = (
        -1
        * K[phi_1_size:assembled_system_size, phi_1_size : phi_1_size + blocksize]
        @ Bl
        @ J21
...
def compute_J(
    K_local: np.ndarray, Bu: np.ndarray, Bl: np.ndarray, phi_1_size: int, blocksize: int
    ...
    J[0:blocksize, blocksize : 2 * blocksize] = (
        K_local[
            phi_1_size : phi_1_size + blocksize, phi_1_size : phi_1_size + blocksize
        ]
        @ Bl
    )
    J[blocksize : 2 * blocksize, 0:blocksize] = (
        K_local[
            phi_1_size - blocksize : phi_1_size, phi_1_size - blocksize : phi_1_size
        ]
        @ Bu
    )
    J[blocksize : 2 * blocksize, blocksize : 2 * blocksize] = np.identity(
        blocksize, dtype=K_local.dtype
    )
...
def update_partition(K_local: np.ndarray, U: np.ndarray):
    ...
    K_local[start_index:stop_index, start_index:stop_index] = (
        K_local[start_index:stop_index, start_index:stop_index] + U
    )
