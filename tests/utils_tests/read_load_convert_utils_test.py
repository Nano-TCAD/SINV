...
def test_create_block_tridiagonal_matrix(
    ...
    rlcu.save_block_tridigonal_matrix(
        "test_matrix.npz",
        diagonal_blocks,
        upper_diagonal_blocks,
        lower_diagonal_blocks,
    ...
    (
        loaded_diagonal_blocks,
        loaded_upper_diagonal_blocks,
        loaded_lower_diagonal_blocks,
    ) = rlcu.read_block_tridiagonal_matrix("test_matrix.npz", is_symmetric=is_symmetric)
...
def test_block_tridiagonal_to_BDIA(
    ...
    BDIA = rlcu.block_tridiagonal_to_BDIA(
        diagonal_blocks,
        upper_diagonal_blocks,
        lower_diagonal_blocks,
        symmetry=bdia_symmetry,
    )
...
@pytest.mark.parametrize(
    "is_symmetric",
    [pytest.param(False, id="non-symmetric"), pytest.param(True, id="symmetric")],
)
@pytest.mark.parametrize(
...
def test_read_local_block_tridiagonal_partition(
    ...
    for i in range(n_partitions):
        (
            ...
            partition_size,
            end_blockrow,
        ) = partu.get_local_partition_indices(i, n_partitions, n_blocks)

        (
            local_diagonal_blocks,
            local_upper_diagonal_blocks,
            local_lower_diagonal_blocks,
        ) = rlcu.read_local_block_tridiagonal_partition(
            "test_matrix.npz",
            start_blockrow,
            partition_size,
            is_symmetric=is_symmetric,
            include_bridges_blocks=include_bridges_blocks,
        )

        if include_bridges_blocks:
            assert np.allclose(
                local_diagonal_blocks,
                diagonal_blocks[start_blockrow:end_blockrow, :, :],
            )
            ...
            assert np.allclose(
                local_upper_diagonal_blocks,
                upper_diagonal_blocks[start_upper_blockrow:stop_upper_blockrow, :, :],
            )
            assert np.allclose(
                local_lower_diagonal_blocks,
                lower_diagonal_blocks[start_lower_blockrow:stop_lower_blockrow, :, :],
            )
        else:
            assert np.allclose(
                local_diagonal_blocks,
            ...
            assert np.allclose(
                local_upper_diagonal_blocks,
                upper_diagonal_blocks[start_blockrow : end_blockrow - 1, :, :],
            )
            assert np.allclose(
                local_lower_diagonal_blocks,
                lower_diagonal_blocks[start_blockrow : end_blockrow - 1, :, :],
            )
