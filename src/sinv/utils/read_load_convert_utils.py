...
def read_local_block_tridiagonal_partition(
    ...
    if include_bridges_blocks:
        ...
        upper_diagonal_blocks = matrix[matrix.files[1]][start_upper_blockrow:stop_upper_blockrow, :, :]
        if is_symmetric:
            lower_diagonal_blocks = np.zeros_like(upper_diagonal_blocks)
            for i in range(upper_diagonal_blocks.shape[0]):
                lower_diagonal_blocks[i, :, :] = upper_diagonal_blocks[i, :, :].T
        else:
            lower_diagonal_blocks = matrix[matrix.files[2]][start_lower_blockrow:stop_lower_blockrow, :, :]

    else:
        upper_diagonal_blocks = matrix[matrix.files[1]][start_blockrow:end_blockrow-1, :, :]
        if is_symmetric:
            lower_diagonal_blocks = np.zeros_like(upper_diagonal_blocks)
            for i in range(upper_diagonal_blocks.shape[0]):
                lower_diagonal_blocks[i, :, :] = upper_diagonal_blocks[i, :, :].T
        else:
            lower_diagonal_blocks = matrix[matrix.files[2]][start_blockrow:end_blockrow-1, :, :]
