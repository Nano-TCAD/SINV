...
def test_rgf2sided(
    is_complex: bool, is_symmetric: bool, matrix_size: int, blocksize: int
):
    ...
    A_diagblk, A_upperblk, A_lowerblk = utils.matu.convertDenseToBlkTridiag(
        A, blocksize
    )
    G_diagblk, G_upperblk, G_lowerblk = rgf2s.rgf2sided(
        A_diagblk, A_upperblk, A_lowerblk, is_symmetric
    )
    ...
    if comm_rank == 0:
        assert (
            np.allclose(A_refsol_diagblk, G_diagblk)
